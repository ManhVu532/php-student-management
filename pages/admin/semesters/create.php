...
if (!empty($_POST)) {
    ...
    if(isset($_POST['id']) && isset($_POST['type']) && isset($_POST['startYear']) && isset($_POST['endYear']) && isset($_POST['fee'])) {
       ...
       if(empty($_POST['id']) || empty($_POST['type']) || empty($_POST['startYear']) || empty($_POST['endYear']) || empty($_POST['fee'])) {
        ...
        if(count($result) > 0){
            echo json_encode(array("status" => "error", "message" => "Mã kỳ học đã tồn tại"));
            exit();
        }

        $query = "INSERT INTO semester_tbl(id, `type`, startYear, endYear, fee) VALUES('" . $id . "','" . $type . "','" . $startYear . "','" . $endYear . "', '".$fee."');";
        executeQuery($query);
