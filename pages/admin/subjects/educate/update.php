...
if (!empty($_POST)) {
    ...
    if (isset($_POST['name']) && isset($_POST['id']) && isset($_POST['numberOfCredits']) && isset($_POST['numberOfLessons'])) {
        ...
        if (count($result) == 0) {
            echo json_encode(array("status" => "error", "message" => "Mã môn học không tồn tại"));
            exit();
        }
    
        $query = "UPDATE subject_tbl SET `name` = '".$name."', numberOfCredits='".$numberOfCredits."', numberOfLessons='".$numberOfLessons."' WHERE id = '".$id."';";
        executeQuery($query);
